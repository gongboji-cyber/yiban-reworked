    ...
    async function load() {
      try {
        ...
        const { data, error } = await supabase
          .from('messages')
          .select('id, content, insight, journal_draft, risk_level, risk_reason, created_at')
          ...
          <div className="history-list">
            {rows.map((item) => (
              <article className="history-item" key={item.id}>
                ...
                <p><strong>当时的表达：</strong>{item.content}</p>
                <p><strong>状态洞察：</strong>{item.insight || '—'}</p>
                <p><strong>记录草稿：</strong>{item.journal_draft || '—'}</p>
                <p><strong>风险说明：</strong>{item.risk_reason || '—'}</p>
              </article>
            ))}
          </div>
        )}
