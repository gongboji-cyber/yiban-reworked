...
export async function POST(req) {
  try {
    ...
    if (!trimmedInput) return NextResponse.json({ error: '输入内容不能为空' }, { status: 400 });

    const ai = await createAiResult(trimmedInput);
    const risk = assessRisk(trimmedInput, ai.emotion_scores || {});
    const server = getServerSupabase();

    const { data: conversationRow, error: conversationError } = await server
      .from('conversations')
      .select('id, user_id')
    ...
    const { error: assistantError } = await server.from('messages').insert({
      user_id: user.id,
      conversation_id: conversationId,
      role: 'assistant',
      content: ai.reply || '',
