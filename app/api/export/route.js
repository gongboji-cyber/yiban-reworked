...
export async function GET(req) {
  try {
    ...
    if (!adminUser || !isAdminEmail(adminUser.email)) {
      return new NextResponse('当前账号没有导出权限', { status: 403 });
    }
    ...
    for (const item of events || []) {
      ...
      const current = userAgg.get(item.user_id) || {
        ...
        consent_school_share: profile.consent_school_share ?? false
      };
    ...
    const audit = workbook.addWorksheet('导出审计');
    audit.columns = [
      { header: '导出时间', key: 'exported_at', width: 24 },
      { header: '操作者邮箱', key: 'operator_email', width: 28 },
      { header: '统计天数', key: 'days', width: 12 },
      { header: '纳入事件数', key: 'event_count', width: 12 },
      { header: '纳入用户数', key: 'user_count', width: 12 }
    ];
    audit.addRow({
      exported_at: new Date().toISOString(),
      operator_email: adminUser.email,
      days,
      event_count: (events || []).length,
      user_count: userAgg.size
    });
    audit.getRow(1).font = { bold: true };

    await server.from('admin_export_logs').insert({
